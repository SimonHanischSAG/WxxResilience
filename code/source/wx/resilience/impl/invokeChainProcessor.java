...

{
	...
	public static final void registerWxxResilience (IData pipeline)
        throws ServiceException
	{
		...
		WxxResilienceProcessor newProcessor = new WxxResilienceProcessor(blacklist);
		String message = "";
		...
		if (processor != null) {
			...
			message = "WxxResilienceProcessor re-registered to InvokeChain using blacklist: " + Arrays.toString(blacklist);
		} else {
			invokeManager.registerProcessor(newProcessor);
			processor = newProcessor;
			message = "WxxResilienceProcessor registered in InvokeChain using blacklist: " + Arrays.toString(blacklist);
		}
		logInfo(message);
	...
	public static final void unregisterWxxResilience (IData pipeline)
        throws ServiceException
	{
		...
		if (processor != null) {
			...
			invokeManager.unregisterProcessor(processor);
			message = "WxxResilienceProcessor unregistered from InvokeChain";
			processor = null;
		} else {
			message = "WxxResilienceProcessor already unregistered from InvokeChain";
		}
		logInfo(message);
		IDataMap pipeMap = new IDataMap(pipeline);
		pipeMap.put("message", message);
	...
	public static WxxResilienceProcessor processor = null;
	
	public static class WxxResilienceProcessor implements InvokeChainProcessor {
	
		private String[] invokeChainBlacklist;
		
		public WxxResilienceProcessor(String[] invokeChainBlacklist) {
		...
		public void process(Iterator chain, BaseService baseService, IData pipeline, ServiceStatus status)
			...
			if (!redirected) {
				...
				boolean executeWxxResilienceServices = false;
				if (isTop) {
					executeWxxResilienceServices = true;
					for (String entry : invokeChainBlacklist) {
						if (baseServiceName.startsWith(entry)) {
							executeWxxResilienceServices = false;
							break;
						}
					}
				}
				//if (isTop || executeWxxResilienceServices)
					//logDebug(baseServiceName + ": " + isTop + "," + executeWxxResilienceServices);
				// we have to make sure that the servcie invocation chain is not cut
				if (chain.hasNext()) {
					try {
						if (executeWxxResilienceServices) {
							try {
	...
	private static void logDebug(String message) {
		JournalLogger.log(4, JournalLogger.FAC_FLOW_SVC, JournalLogger.INFO, "WxxResilienceProcessor", message);
	}
	...
	private static void logError(String message) {
		JournalLogger.log(4, JournalLogger.FAC_FLOW_SVC, JournalLogger.ERROR, "WxxResilienceProcessor", message);
	}
	
		
	private static final String REDIRECTED = "@WxxResilience.service.redirected@";
	private static final String WRAPPED = "@WxxResilience.service.wrapped@";
	private static final String POSTPROCESS_EXECUTED = "@WxxResilience.postProcess.executed@";
